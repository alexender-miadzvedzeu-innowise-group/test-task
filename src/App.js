...
export default function App() {
  ...
  useEffect(() => {
    const rotateList = setInterval(() => {
      if (list.length > MAX_LENGTH) {
        ...
        setList([...list.slice(1, list.length), ...list.slice(0, 1)]);
      }
    }, DURATION);
  ...
  const loadSongsList = async (name) => {
    try {
      const response = await fetch(
        `https://itunes.apple.com/search?term=${name}&limit=${MAX_LENGTH}`
      );
      const body = await response.json();
      if (body.results) {
        const sortedList = body.results
          .sort((a, b) => b.collectionName - a.collectionName)
          .reduce(
            (res, val) =>
              val.collectionName ? [...res, val.collectionName] : [...val],
            []
          );
  ...
  useEffect(() => {
    if (inputValue) loadSongsList(inputValue);
  }, [inputValue]);
    ...
    return (
      <ul className="list">
        {list.map(
          (el, index) =>
            index < MAX_LENGTH && (
              <li className="list-item" key={guid()}>
                {el}
              </li>
            )
        )}
